...
#ifndef ONDEVICE_CORE_H
#define ONDEVICE_CORE_H
...
// Read the main app's name, Bundle ID, version, and optional PNG icon from an
// IPA. On success `out_json` contains UTF-8 JSON and must be freed with
// si_string_free. This operation does not authenticate or sign the IPA.
int32_t si_ipa_metadata(const char *ipa_path,
                        char **out_json,
                        char **out_error);
