...
#[cfg(feature = "install")]
use crate::dev::devices::DevicesApi;
...
impl Sideloader {
    ...
    pub async fn sign_app(
        ...
        let original_bundle_id = app.main_bundle_id()?;

        if let Some(bundle_id) = bundle_id_override.filter(|value| !value.is_empty()) {
            app.bundle.set_bundle_identifier(bundle_id);
        }
        if let Some(display_name) = display_name_override.filter(|value| !value.is_empty()) {
        ...
        // Extensions still carry the IPA's original identifier prefix. Rewrite
        // that prefix to the edited, team-qualified main identifier as one step.
        app.update_bundle_id(&original_bundle_id, &main_app_id_str)?;
        let mut app_ids = app
            .register_app_ids(
    ...
    pub async fn install_app(
        ...
        let (signed_app_path, special_app) = self
            .sign_app(app_path, Some(team), increased_memory_limit, None, None)
            .await?;
