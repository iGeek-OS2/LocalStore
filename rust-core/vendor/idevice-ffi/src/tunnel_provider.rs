...
pub unsafe extern "C" fn tunnel_create_rppairing(
    ...
    hostname: *const c_char,
    pairing_file: *mut RpPairingFileHandle,
    _pin_callback: Option<extern "C" fn(context: *mut c_void) -> *const c_char>,
    _pin_context: *mut c_void,
    out_adapter: *mut *mut AdapterHandle,
    out_handshake: *mut *mut RsdHandshakeHandle,
) -> *mut IdeviceFfiError {
    if addr.is_null()
    ...
    let res = run_sync_local(async {
        ...
        let mut rpc = RemotePairingClient::new(conn, &host);
        // This entry point is exclusively for an existing pair record. The
        // generic `connect` helper falls back to initial pairing when verify
        // fails, which hides a revoked record from Swift and leaves the UI in
        // the paired state. Propagate PairVerifyFailed (103/5) instead.
        rpc.attempt_pair_verify().await?;
        rpc.validate_pairing(rpf).await?;
